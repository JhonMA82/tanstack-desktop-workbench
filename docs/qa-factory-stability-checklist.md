...
| # | Escenario | Comando | Resultado esperado |
|---|---|---|---|
| 6.1 | Self-test del scaffolder | `bun run self-test:scaffolding` | Exit 0. Genera 8 derivados reales (uno por preset + custom with/without), verifica en cada uno la poda (sin `showcase`, sin presets no elegidos, sin theme no elegido, router sin demo) y les corre typecheck (vía symlink a `node_modules`, sin red, con tiempos por caso) más un loop `generate:feature` → `ai:context` dentro del derivado; limpia las fixtures. Cubre 2.1–2.4 y 2.11 de forma automática. |
